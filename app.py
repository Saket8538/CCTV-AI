...
class MainScreen:
    ...
    def _load_notification_config(self):
        ...
        # Prefer Streamlit secrets config if available.
        if tomllib is not None and path.isfile('./.streamlit/secrets.toml'):
            try:
                with open('./.streamlit/secrets.toml', 'rb') as f:
                    sec = tomllib.load(f)
                cfg["email"] = sec.get("email", {}) if isinstance(sec, dict) else {}
                cfg["twilio"] = sec.get("twilio", {}) if isinstance(sec, dict) else {}
            except Exception as e:
                logging.error(f"Failed reading .streamlit/secrets.toml: {e}")

        # Fallback to legacy creds.json for twilio.
        if (not cfg["twilio"]) and path.isfile('./creds.json'):
            try:
                legacy = loads(open('creds.json').read())
                if isinstance(legacy, dict):
                    cfg["twilio"] = legacy.get("twilio", {})
            except Exception as e:
                logging.error(f"Failed reading creds.json: {e}")

        return cfg

    def _normalize_phone(self, to):
        digits = ''.join(ch for ch in str(to) if ch.isdigit() or ch == '+')
    ...
    def send__sms(self, to, body):
        cfg = self._load_notification_config()
        tw_creds = cfg.get("twilio", {})

        account_sid = tw_creds.get("sid")
        auth_token = tw_creds.get("token")
        if not account_sid or not auth_token:
            raise ValueError("Twilio sid/token not configured")
        ...
        messaging_service_sid = tw_creds.get('messaging_service_sid')
        if messaging_service_sid:
            payload['messaging_service_sid'] = messaging_service_sid
        elif tw_creds.get('from_'):
            payload['from_'] = tw_creds['from_']
        else:
            raise ValueError("Twilio from_ or messaging_service_sid not configured")

        message = client.messages.create(**payload)
        return message.sid

    def send__email(self, to_email, subject, body):
        cfg = self._load_notification_config()
        em_creds = cfg.get("email", {})

        sender = em_creds.get("sender")
        password = em_creds.get("password")
        smtp_host = em_creds.get("smtp_host", "smtp.gmail.com")
        smtp_port = int(em_creds.get("smtp_port", 587))
        team_to = em_creds.get("to")

        recipients = []
        if team_to:
            recipients.append(str(team_to).strip())
        if to_email:
            recipients.append(str(to_email).strip())
        recipients = [r for r in dict.fromkeys(recipients) if r]

        if not sender or not password or not recipients:
            raise ValueError("Email sender/password/recipient not configured")
    ...
    def match(self):
        ...
        try:
            ...
            try:
                ...
                sms_ok = bool(response)
                sms_msg = "SMS sent successfully."
            except Exception as sms_err:
                logging.error(f"SMS notification failed: {sms_err}")
                sms_msg = "SMS failed."

            try:
                complaint_email = fetch['complaint_email'] if 'complaint_email' in fetch.keys() else ""
                recipients = self.send__email(
                    complaint_email,
                    subject=f"Missing Person Located - Case ID {pid}",
                    body=msg_tmplt,
                )
                email_ok = len(recipients) > 0
                email_msg = "Email notification sent."
            except Exception as mail_err:
                logging.error(f"Email notification failed: {mail_err}")
            ...
            if sms_ok or email_ok:
                ...
                db.commit()
                self.update_verification_count()
                messagebox.showinfo("Success!!", f"{sms_msg} {email_msg}", parent=root)
                self.refresh()
            else:
                messagebox.showinfo("Error!!", "SMS and Email could not be sent.", parent=root)
        except Exception as e:
